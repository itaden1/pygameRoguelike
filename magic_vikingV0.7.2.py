        ...
        if target.category == 'spawner':
            ...
            if hit_chance >= miss_chance:
                ...
                if dmg <= 0:
                    dmg = 1
                target.hp -= dmg
                if self.name == 'spider' or self.name == 'spider_queen':
...
def create_spider_room(room):
    ...
    for i in range(max_spiders):
        ...
        if random_roll <= 10:
            ...
            per = 0
            lab = 0
            weapon = Equipment('fang',knife_img,2,0,0,0,3,0,0,0,0,0,0,5,0,0,0,0,'dagger')
            armour = None
            x = random.randint(room.x1+1,room.x2-1)
            y = random.randint(room.y1+1,room.y2-1)
            ...
            if bosses == 0:
                ...
                per = 0
                lab = 0
                weapon = Equipment('fang',dagger_img,15,0,0,0,9,0,0,0,0,0,0,9,0,0,0,0,'weapon')
                armour = None
                x = random.randint(room.x1+1,room.x2-1)
                y = random.randint(room.y1+1,room.y2-1)
